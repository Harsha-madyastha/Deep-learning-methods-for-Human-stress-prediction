...
#     def __init__(self, params):
#         self.params = params
#         super(MemoryFusionNetwork, self).__init__()
#
#         self.hidden_dim = params.d_rnn
#         self.num_layers = params.rnn_n_layers
#         self.output_dim = params.n_targets
#
#         self.shared_layer_1 = nn.Linear(self.params.d_in, self.hidden_dim, nn.ReLU(True))
#         self.shared_layer_2 = nn.Linear(self.hidden_dim, self.hidden_dim, nn.ReLU(True))
#
#         self.memory_layer = nn.LSTM(self.hidden_dim, self.hidden_dim, num_layers=self.num_layers)
#
#         self.valence_layer = nn.Linear(self.hidden_dim, self.output_dim)
#
#     def forward(self, x, x_len):
#         # Concatenate the inputs along the feature dimension
#         # inputs = torch.cat([faus, egemaps, biosignals], dim=1)
#         # x = self.inp(x)
#         inputs = x.clone()
#
...
#         # x = nn.ReLU(x)
#
#         # Reshape the shared layer output to (sequence_length, batch_size, hidden_dim)
#         x = x.view(inputs.shape[0], -1, self.hidden_dim)
#
#         # Initialize the memory cell and hidden state
#         h0 = torch.zeros(self.num_layers, inputs.shape[0], self.hidden_dim).to(inputs.device)
#         c0 = torch.zeros(self.num_layers, inputs.shape[0], self.hidden_dim).to(inputs.device)
#         x_packed = pack_padded_sequence(x, x_len.cpu(), batch_first=True, enforce_sorted=False)
#
#         # Pass the shared layer output through the memory layer
#         rnn_enc = self.memory_layer(x_packed, (h0, c0))
#         x_out = rnn_enc[0]
#         x_out = pad_packed_sequence(x_out, total_length=x.size(1), batch_first=True)[0]
#
#         # Use the final memory cell state as input to the output layer
#         # valence_output = self.valence_layer(hn[-1])
#         valence_output = self.valence_layer(x_out)
#
#

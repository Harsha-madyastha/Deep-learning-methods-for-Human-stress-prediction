...
def calc_ccc(preds, labels):
    ...
    #
    # ccc=accuracy(preds.reshape(-1,3), labels.reshape(-1,1))
    predict = preds.reshape(-1,3).argmax(-1)
...
def evaluate(task, model, data_loader, loss_fn, eval_fn, use_gpu=False, predict=False, prediction_path=None, filename=None):
    ...
    with torch.no_grad():
        for batch, batch_data in enumerate(data_loader, 1):
            ...
            # loss = loss_fn(preds, labels[:,-1,:][0])
            # loss = loss_fn(preds, labels[:, -1, :].squeeze(-1).squeeze(-1))
            loss = loss_fn(preds, labels[:, -1, :].squeeze(-1).squeeze(-1).type(torch.LongTensor))

            losses += loss.item() * batch_size
            sizes += batch_size

            full_labels.append(labels[:,-1,:].squeeze(-1).squeeze(-1).cpu().detach().squeeze().numpy().tolist())

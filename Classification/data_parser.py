...
def segment_stress(sample, win_len, hop_len):
    segmented_sample = []
    assert hop_len <= win_len and win_len >= 3
...
def load_stress_subject(feature, subject_id, partition, emo_dim, normalizer, apply_segmentation=True,
    ...
    # label_data = pd.DataFrame(data=df[['labels_Neutral','labels_Non_stressed','labels_Stressed']].values, columns=['labels_Neutral','labels_Non_stressed','labels_Stressed'])
    label_data = pd.DataFrame(data=df['label'].values, columns=[emo_dim])
    # label_data = df[['Neutral','Non_stressed','Stressed']]
...
def load_data(task, paths, feature, emo_dim, normalize=True, win_len=200, hop_len=100, save=False,
    ...
    if os.path.exists(data_file):
        print(data_file)# check if file of preprocessed data exists

...
class OutLayer(nn.Module):
    def __init__(self, d_in, d_hidden, d_out, dropout=.2, bias=.0):
...
class Model(nn.Module):
    def __init__(self, params):
        ...
        self.out1 = OutLayer(d_rnn_out, params.d_fc_out, params.n_targets, dropout=params.linear_dropout)
        self.out2 = OutLayer(d_rnn_out, params.d_fc_out, params.n_targets, dropout=params.linear_dropout)
    ...
    def forward(self, x, x_len):
        ...
        if self.params.rnn_n_layers > 0:
            x = self.rnn(x, x_len)
        val_output = self.out1(x)
        arousal_output = self.out2(x)
        val_output = self.final_activation(val_output)
        arousal_output = self.final_activation(arousal_output)
        return val_output, arousal_output

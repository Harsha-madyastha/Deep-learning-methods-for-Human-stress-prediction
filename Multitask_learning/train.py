...
def train_model(task, model, data_loader, epochs, lr, model_path, current_seed, use_gpu, loss_fn, eval_fn,
                eval_metric_str, early_stopping_patience, reduce_lr_patience, primary_task,regularization=0.0):
    train_loader, val_loader = data_loader['train'], data_loader['devel']
    ...
    for epoch in range(1, epochs + 1):
        ...
        loss, val_score, arousal_score = evaluate(task, model, val_loader, loss_fn=loss_fn, eval_fn=eval_fn, use_gpu=use_gpu)

        print(f'Epoch:{epoch:>3} / {epochs} | [Train] | Loss: {train_loss:>.4f}')
        print(f'Epoch:{epoch:>3} / {epochs} | [Val] | Loss: {loss:>.4f} | [{eval_metric_str}]: {val_score:>7.4f} | [{eval_metric_str}]: {arousal_score:>7.4f}')
        print('-' * 50)

        if primary_task=='valence':
            ...
            if arousal_score > best_arousal_score:
                early_stop = 0
                best_arousal_score = arousal_score
                best_val_loss = loss
                best_model_file = save_model(model, model_path, current_seed)
    ...
    if primary_task=='valence':
        ...
        print(f'Seed {current_seed} | '
              f'Best [Val {eval_metric_str}]:{best_arousal_score:>7.4f} | Loss: {best_val_loss:>.4f}')
        return best_val_loss, best_arousal_score, best_model_file

...
def parse_args():
    ...
    parser.add_argument('--primary_task', type=str, default='valence',
...
def main(args):
    ...
    if args.eval_model is None:  # Train and validate for each seed
        ...
        losses, scores, best_model_files, test_scores = [], [], [], []
        ...
        for seed in seeds:
            ...
            model = Model(args)
            # x = torch.randn(205,200,88)
            # x_len = torch.randint(low=1, high=201, size=(205,),
            #                       dtype=torch.long)  # randomly generate sequence lengths between 1 and 200 for each sample in the batch
            # make_dot(model(x,x_len), params=dict(model.named_parameters()))
            print('=' * 50)
            print(f'Training model... [seed {seed}] for at most {args.epochs} epochs')

            loss, score, best_model_file = train_model(args.task, model, data_loader, args.epochs,
                                                               args.lr, args.paths['model'], seed, use_gpu=args.use_gpu,
                                                               primary_task=args.primary_task,loss_fn=loss_fn, eval_fn=eval_fn,
        ...
        best_idx = scores.index(max(scores))  # find best performing seed



        print('=' * 50)
        print(f'Best {eval_str} on [Val] for seed {seeds[best_idx]}: '
              f'[Val {eval_str}]: {scores[best_idx]:7.4f}'
        ...
        _, valid_score, arousal_score = evaluate(args.task, model, data_loader['devel'], loss_fn=loss_fn, eval_fn=eval_fn,
                                  use_gpu=args.use_gpu)
        print(f'Evaluating {model_file}:')
        print(f'[Val {eval_str}]: {valid_score:7.4f}, {eval_str}]: {arousal_score:7.4f}')
        if not args.predict:
            _, test_score = evaluate(args.task, model, data_loader['devel'], loss_fn=loss_fn, eval_fn=eval_fn,
                                     use_gpu=args.use_gpu)
            print(f'[Test {eval_str}]: {test_score:7.4f}')
...
if __name__ == '__main__':
    ...
    args.log_file_name = '{}_[{}]_[{}]_[{}_{}_{}_{}]_[{}_{}]_[{}]_[{}_{}]'.format(
        datetime.now(tz=tz.gettz()).strftime("%Y-%m-%d-%H-%M"), args.feature, args.emo_dim,
        args.d_rnn, args.rnn_n_layers, args.rnn_bi, args.d_fc_out, args.lr, args.batch_size,args.primary_task,args.a,args.b) if args.task == 'stress' else \

...
def evaluate(task, model, data_loader, loss_fn, eval_fn, use_gpu=False, predict=False, prediction_path=None, filename=None):
    ...
    with torch.no_grad():
        for batch, batch_data in enumerate(data_loader, 1):
            ...
            loss_val = loss_fn(preds[0].squeeze(-1), labels[:, :, 0].squeeze(-1), feature_lens)
            loss_arousal = loss_fn(preds[1].squeeze(-1), labels[:, :, 1].squeeze(-1), feature_lens)

            loss=torch.mul(loss_val, model.a)+torch.mul(loss_arousal, model.b)

            losses += loss.item() * batch_size
            sizes += batch_size

            full_labels_val.append(labels[:,:,0].cpu().detach().squeeze().numpy().tolist()[:cutoff])
            full_labels_arousal.append(labels[:,:,1].cpu().detach().squeeze().numpy().tolist()[:cutoff])

            full_preds_val.append(preds[0].cpu().detach().squeeze().numpy().tolist()[:cutoff])
            full_preds_arousal.append(preds[1].cpu().detach().squeeze().numpy().tolist()[:cutoff])


        if predict:
            write_predictions(task, full_metas, full_preds_val, prediction_path, "predictions_val.csv")
            write_predictions(task, full_metas, full_preds_arousal, prediction_path, "predicitons_arousal.csv")
            return
        else:
            if task == 'stress':
                full_preds_val = flatten_stress_for_ccc(full_preds_val)
                full_preds_arousal = flatten_stress_for_ccc(full_preds_arousal)
                full_labels_val = flatten_stress_for_ccc(full_labels_val)
                full_labels_arousal = flatten_stress_for_ccc(full_labels_arousal)

            score_val = eval_fn(full_preds_val, full_labels_val)
            score_arousal = eval_fn(full_preds_arousal, full_labels_arousal)
            total_loss = losses / sizes
            return total_loss, score_val , score_arousal
